...
#
# Environment:
#   AUDIOCFG_SRCDIR       project root (default: packaging/../..)
#   AUDIOCFG_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
AUDIOCFG_SRCDIR=${AUDIOCFG_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$AUDIOCFG_SRCDIR/.config/audiocfg" "$HOME/.config/audiocfg"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${AUDIOCFG_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${AUDIOCFG_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${AUDIOCFG_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $AUDIOCFG_SRCDIR/.config/audiocfg/$k.build-host" >&2
        echo "    $HOME/.config/audiocfg/$k.build-host" >&2
    done
    return 2
}
